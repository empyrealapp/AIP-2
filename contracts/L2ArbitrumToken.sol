...
import "./lib/RecoveryLib.sol"; // adds unstructured storage lib
    ...
    TransferAndCallToken
{
    ...
    function initializeRecovery(
        ...
        require(!RecoveryLib.hasInitializedRecovery(), "recovery already set");
        require(addresses.length == amounts.length, "recover array mismatch");
        for (uint i = 0; i < addresses.length; i++) {
            RecoveryLib.setRecoverForUser(addresses[i], amounts[i]);
        }
        RecoveryLib.initializeRecovery();
    }

    function recover() external {
        require(RecoveryLib.hasInitializedRecovery(), "recovery not setup yet");
        require(!RecoveryLib.hasRecovered(msg.sender), "already recovered");

        uint amount = RecoveryLib.recoverAmount(msg.sender);
        require(amount > 0, "no recover balance");
        transfer(msg.sender, amount);

        RecoveryLib.setRecovered(msg.sender);
    }
}
